#define FOSC 8000000UL              // system clock frequency
...
void PWM_init(uint16_t dcyc){
    ...
    OC1CON2 = 0x00;                 // ^^^
    OC1CON1bits.OCM = 0b000;        // Output compare channel is disabled
...
void PWM_updDcyc(uint16_t dcyc){
    if(OC1CON1bits.OCM == 0)     OC1CON1bits.OCM = 0b110;
    if(dcyc > 100)  dcyc=100;
    if(dcyc == 0)   OC1CON1bits.OCM = 0;
